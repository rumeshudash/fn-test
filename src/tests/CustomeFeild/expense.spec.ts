...
test.describe('Configurations-Custom Feilds', () => {
    PROCESS_TEST('TCF001- Expense -Negative', async ({ page }) => {
        ...
        await PROCESS_TEST.step('Check Expense Tab', async () => {
        ...
        await PROCESS_TEST.step('Check Add New Button', async () => {
            ...
            expect(await dialog.getDialogTitle()).toBe(
                'Add Expense Custom Field'
            );
        });

        await PROCESS_TEST.step('Add Expense With Empty Feilds', async () => {
